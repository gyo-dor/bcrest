...
def text(img, text):
    ...
    d.text((32, img.size[1]-32), text, font=fnt, fill=(255,255,255,196))
    out = Image.alpha_composite(base, txt)
    return out

def paint(filename):
    img = Image.open('static/res/temp_cv_img.png')
    original_size = img.size

    if (original_size[0] < 33) | (original_size[1] < 33):
        return 'failed'
    ...
    img = img.convert('RGB')
    img.save('static/res/temp_painter_img.jpg')
    # Paint the image
    img = generate('static/res/temp_painter_img.jpg')
    # Return the processed image to original size
    img = img.resize(original_size)
    img = text(img, "skakmat systems")
    img.save('static/rout/' + filename)
